...
const CREDIT_NAMES = ['Interest Coverage Ratio', 'DSCR', 'Debt/EBITDA', 'Debt/Capital', 'Loan-to-Value (LTV)', 'Debt-to-Income Ratio', 'Fixed Charge Coverage Ratio', 'Cash Flow Coverage Ratio', 'Current Ratio', 'Quick Ratio', 'Cash Ratio', 'Altman Z-Score', 'Piotroski F-Score', 'Credit Scorecard', 'Probability of Default', 'Expected Loss', 'Loss Given Default', 'Exposure at Default'];
const DERIVATIVE_NAMES = ['Forward Contract', 'Futures P&L', 'Options Profit/Loss', 'Call Option Payoff', 'Put Option Payoff', 'Put-Call Parity', 'Option Break-Even', 'Black-Scholes', 'Option Greeks', 'Delta', 'Gamma', 'Theta', 'Vega', 'Rho'];
const CREDIT = CREDIT_NAMES.map((name) => [slugify(name), `${name} Calculator`, 'Credit & banking', `Calculate ${name.toLowerCase()} from your credit inputs.`]);
const DERIVATIVES = DERIVATIVE_NAMES.map((name) => [slugify(name), `${name} Calculator`, 'Derivatives', `Calculate ${name.toLowerCase()} from your derivatives inputs.`]);
const M_AND_A_NAMES = ['Purchase Price Allocation', 'Goodwill', 'Enterprise Value to Equity Value Bridge', 'Accretion/Dilution', 'Merger Exchange Ratio', 'Stock-for-Stock Merger', 'Cash Acquisition', 'M&A Synergy', 'LBO', 'LBO Returns', 'Sponsor IRR', 'MOIC'];
const M_AND_A = M_AND_A_NAMES.map((name) => [slugify(name), `${name} Calculator`, 'M&A / corporate transactions', `Calculate ${name.toLowerCase()} from your transaction inputs.`]);
const TOOLS = [...PERSONAL, ...TAX, ...BUSINESS, ...STARTUP, ...DATE_TIME, ...TIME_VALUE, ...INVESTMENT, ...VALUATION, ...ACCOUNTING, ...FIXED_INCOME, ...REAL_ESTATE, ...CREDIT, ...DERIVATIVES, ...M_AND_A];
...
function fieldsFor(id) {
  ...
  if (['interest-coverage-ratio', 'dscr', 'debt-ebitda', 'debt-capital', 'fixed-charge-coverage-ratio', 'cash-flow-coverage-ratio'].includes(id)) return [input('ebit', 'EBIT / operating profit', '₹', 2000000, '0.01'), input('interest', 'Interest expense', '₹', 400000, '0.01'), input('debt', 'Total debt', '₹', 5000000, '0.01'), input('ebitda', 'EBITDA', '₹', 2500000, '0.01'), input('capital', 'Total capital', '₹', 10000000, '0.01'), input('cashFlow', 'Cash flow available', '₹', 1800000, '0.01'), input('debtService', 'Debt service', '₹', 900000, '0.01'), input('fixedCharges', 'Fixed charges', '₹', 300000, '0.01')];
  if (id === 'loan-to-value-ltv') return [input('loan', 'Loan amount', '₹', 4000000, '0.01'), input('propertyValue', 'Property value', '₹', 6000000, '0.01')];
  if (id === 'debt-to-income-ratio') return [input('monthlyDebt', 'Monthly debt payments', '₹', 30000, '0.01'), input('monthlyIncome', 'Gross monthly income', '₹', 100000, '0.01')];
  if (['current-ratio', 'quick-ratio', 'cash-ratio'].includes(id)) return [input('currentAssets', 'Current assets', '₹', 600000, '0.01'), input('inventory', 'Inventory', '₹', 150000, '0.01'), input('cash', 'Cash and equivalents', '₹', 200000, '0.01'), input('currentLiabilities', 'Current liabilities', '₹', 350000, '0.01')];
  if (id === 'altman-z-score') return [input('workingCapital', 'Working capital', '₹', 250000, '0.01'), input('assets', 'Total assets', '₹', 2000000, '0.01'), input('retainedEarnings', 'Retained earnings', '₹', 500000, '0.01'), input('ebit', 'EBIT', '₹', 300000, '0.01'), input('marketEquity', 'Market value of equity', '₹', 3000000, '0.01'), input('liabilities', 'Total liabilities', '₹', 1000000, '0.01'), input('sales', 'Sales', '₹', 4000000, '0.01')];
  if (id === 'piotroski-f-score') return [input('signalsPositive', 'Positive accounting signals', 'signals', 6, '1'), input('signalsTotal', 'Total signals tested', 'signals', 9, '1')];
  if (id === 'credit-scorecard') return [input('income', 'Annual income', '₹', 1200000, '0.01'), input('debt', 'Total debt', '₹', 3000000, '0.01'), input('latePayments', 'Late payments', 'count', 1, '1'), input('yearsCredit', 'Credit history', 'years', 5, '0.01')];
  if (id === 'probability-of-default') return [input('creditScore', 'Credit score', 'score', 700, '0.01'), input('debtRatio', 'Debt ratio', '%', 40, '0.0001'), input('latePayments', 'Late payments', 'count', 1, '1')];
  if (id === 'expected-loss') return [input('pd', 'Probability of default', '%', 3, '0.0001'), input('lgd', 'Loss given default', '%', 45, '0.0001'), input('ead', 'Exposure at default', '₹', 1000000, '0.01')];
  if (id === 'loss-given-default') return [input('exposure', 'Exposure at default', '₹', 1000000, '0.01'), input('recovery', 'Recovery amount', '₹', 550000, '0.01')];
  if (id === 'exposure-at-default') return [input('drawn', 'Current drawn balance', '₹', 500000, '0.01'), input('undrawn', 'Undrawn commitment', '₹', 300000, '0.01'), input('creditConversion', 'Credit conversion factor', '%', 75, '0.0001')];
  if (['forward-contract', 'futures-p-l'].includes(id)) return [input('spotPrice', 'Spot price', '₹', 100, '0.01'), input('contractPrice', 'Contract price', '₹', 105, '0.01'), input('contracts', 'Number of contracts', 'contracts', 10, '1'), input('contractSize', 'Units per contract', 'units', 100, '0.01')];
  if (['options-profit-loss', 'call-option-payoff', 'put-option-payoff', 'option-break-even', 'black-scholes', 'option-greeks', 'delta', 'gamma', 'theta', 'vega', 'rho'].includes(id)) return [input('spot', 'Underlying price', '₹', 100, '0.01'), input('strike', 'Strike price', '₹', 105, '0.01'), input('volatility', 'Volatility', '%', 20, '0.0001'), input('riskFree', 'Risk-free rate', '%', 6, '0.0001'), input('years', 'Time to expiry', 'years', 1, '0.01'), input('premium', 'Option premium', '₹', 8, '0.01')];
  if (id === 'put-call-parity') return [input('call', 'Call price', '₹', 8, '0.01'), input('put', 'Put price', '₹', 7, '0.01'), input('spot', 'Underlying price', '₹', 100, '0.01'), input('strike', 'Strike price', '₹', 105, '0.01'), input('riskFree', 'Risk-free rate', '%', 6, '0.0001'), input('years', 'Time to expiry', 'years', 1, '0.01')];
  if (id === 'purchase-price-allocation') return [input('purchasePrice', 'Purchase consideration', '₹', 50000000, '0.01'), input('tangibleAssets', 'Fair value of tangible assets', '₹', 40000000, '0.01'), input('liabilities', 'Liabilities assumed', '₹', 15000000, '0.01'), input('intangibles', 'Identifiable intangible assets', '₹', 10000000, '0.01'), input('deferredTax', 'Deferred tax liabilities', '₹', 2000000, '0.01')];
  if (id === 'goodwill') return [input('purchasePrice', 'Purchase consideration', '₹', 50000000, '0.01'), input('netAssets', 'Fair value of identifiable net assets', '₹', 43000000, '0.01')];
  if (id === 'enterprise-value-to-equity-value-bridge') return [input('enterpriseValue', 'Enterprise value', '₹', 60000000, '0.01'), input('debt', 'Debt', '₹', 15000000, '0.01'), input('cash', 'Cash and equivalents', '₹', 5000000, '0.01'), input('minorityInterest', 'Minority interest', '₹', 0, '0.01'), input('investments', 'Non-operating investments', '₹', 0, '0.01')];
  if (id === 'accretion-dilution') return [input('buyerNetIncome', 'Buyer net income', '₹', 10000000, '0.01'), input('buyerShares', 'Buyer shares', 'shares', 1000000, '0.01'), input('targetNetIncome', 'Target net income', '₹', 3000000, '0.01'), input('sharesIssued', 'Shares issued', 'shares', 250000, '0.01'), input('synergies', 'Annual pre-tax synergies', '₹', 1000000, '0.01'), input('taxRate', 'Tax rate', '%', 25, '0.0001'), input('financingCost', 'Financing cost', '₹', 300000, '0.01')];
  if (id === 'merger-exchange-ratio') return [input('targetSharePrice', 'Target share price', '₹', 120, '0.01'), input('buyerSharePrice', 'Buyer share price', '₹', 150, '0.01'), input('premium', 'Offer premium', '%', 10, '0.0001')];
  if (id === 'stock-for-stock-merger') return [input('targetShares', 'Target shares', 'shares', 1000000, '0.01'), input('exchangeRatio', 'Exchange ratio', 'x', 0.8, '0.0001'), input('buyerSharePrice', 'Buyer share price', '₹', 150, '0.01')];
  if (id === 'cash-acquisition') return [input('offerPrice', 'Cash offer per share', '₹', 120, '0.01'), input('targetShares', 'Target shares', 'shares', 1000000, '0.01'), input('debtAssumed', 'Debt assumed', '₹', 5000000, '0.01'), input('cashAcquired', 'Cash acquired', '₹', 2000000, '0.01')];
  if (id === 'm-a-synergy') return [input('revenueSynergy', 'Annual revenue synergy', '₹', 3000000, '0.01'), input('costSynergy', 'Annual cost synergy', '₹', 2000000, '0.01'), input('implementationCost', 'One-time implementation cost', '₹', 1000000, '0.01'), input('taxRate', 'Tax rate', '%', 25, '0.0001'), input('years', 'Synergy period', 'years', 5, '0.01')];
  if (['lbo', 'lbo-returns'].includes(id)) return [input('entryEV', 'Entry enterprise value', '₹', 100000000, '0.01'), input('debtFinancing', 'Debt financing', '₹', 60000000, '0.01'), input('equityContribution', 'Sponsor equity contribution', '₹', 40000000, '0.01'), input('exitEV', 'Exit enterprise value', '₹', 180000000, '0.01'), input('debtRepaid', 'Debt repaid at exit', '₹', 30000000, '0.01'), input('years', 'Holding period', 'years', 5, '0.01')];
  if (id === 'sponsor-irr') return [input('investment', 'Sponsor investment', '₹', 40000000, '0.01'), input('proceeds', 'Sponsor exit proceeds', '₹', 100000000, '0.01'), input('years', 'Holding period', 'years', 5, '0.01')];
  if (id === 'moic') return [input('investment', 'Invested capital', '₹', 40000000, '0.01'), input('proceeds', 'Exit proceeds', '₹', 100000000, '0.01')];
...
function calculate(id, v) {
  ...
  if (id === 'purchase-price-allocation') { const fairNetAssets = v.tangibleAssets + v.intangibles - v.liabilities - v.deferredTax; return result(v.purchasePrice - fairNetAssets, v.purchasePrice, fairNetAssets, 'Purchase consideration', 'Fair value of net assets'); }
  if (id === 'goodwill') return result(v.purchasePrice - v.netAssets, v.purchasePrice, v.netAssets, 'Purchase consideration', 'Identifiable net assets');
  if (id === 'enterprise-value-to-equity-value-bridge') return result(v.enterpriseValue - v.debt - v.minorityInterest + v.cash + v.investments, v.enterpriseValue, v.debt - v.cash, 'Enterprise value', 'Debt less cash');
  if (id === 'accretion-dilution') { const standaloneEPS = v.buyerNetIncome / v.buyerShares; const proFormaEPS = (v.buyerNetIncome + v.targetNetIncome + v.synergies * (1 - v.taxRate / 100) - v.financingCost) / (v.buyerShares + v.sharesIssued); return result((proFormaEPS / standaloneEPS - 1) * 100, proFormaEPS, standaloneEPS, 'Pro forma EPS', 'Standalone buyer EPS', 'percent', 'money', 'money'); }
  if (id === 'merger-exchange-ratio') return result(v.targetSharePrice * (1 + v.premium / 100) / v.buyerSharePrice, v.targetSharePrice, v.buyerSharePrice, 'Target share price', 'Buyer share price', 'number', 'money', 'money');
  if (id === 'stock-for-stock-merger') return result(v.targetShares * v.exchangeRatio * v.buyerSharePrice, v.targetShares * v.exchangeRatio, v.buyerSharePrice, 'New buyer shares', 'Buyer share price', 'money', 'number', 'money');
  if (id === 'cash-acquisition') return result(v.offerPrice * v.targetShares + v.debtAssumed - v.cashAcquired, v.offerPrice * v.targetShares, v.debtAssumed - v.cashAcquired, 'Equity purchase price', 'Debt less cash');
  if (id === 'm-a-synergy') { const annualAfterTax = (v.revenueSynergy + v.costSynergy) * (1 - v.taxRate / 100); return result(annualAfterTax * v.years - v.implementationCost, annualAfterTax, v.implementationCost, 'Annual after-tax synergy', 'Implementation cost'); }
  if (['lbo', 'lbo-returns'].includes(id)) { const exitEquity = v.exitEV - v.debtRepaid; const moic = exitEquity / v.equityContribution; return result((moic ** (1 / v.years) - 1) * 100, moic, exitEquity, 'MOIC', 'Exit equity value', 'percent', 'number', 'money'); }
  if (id === 'sponsor-irr') return result(((v.proceeds / v.investment) ** (1 / v.years) - 1) * 100, v.investment, v.proceeds, 'Sponsor investment', 'Exit proceeds', 'percent', 'money', 'money');
  if (id === 'moic') return result(v.proceeds / v.investment, v.investment, v.proceeds, 'Invested capital', 'Exit proceeds', 'number');
  if (['interest-coverage-ratio', 'dscr', 'debt-ebitda', 'debt-capital', 'fixed-charge-coverage-ratio', 'cash-flow-coverage-ratio'].includes(id)) {
    if (id === 'interest-coverage-ratio') return result(v.ebit / v.interest, v.ebit, v.interest, 'EBIT', 'Interest expense', 'number');
    if (id === 'dscr') return result(v.cashFlow / v.debtService, v.cashFlow, v.debtService, 'Cash flow available', 'Debt service', 'number');
    if (id === 'debt-ebitda') return result(v.debt / v.ebitda, v.debt, v.ebitda, 'Total debt', 'EBITDA', 'number');
    if (id === 'debt-capital') return result(v.debt / v.capital * 100, v.debt, v.capital, 'Total debt', 'Total capital', 'percent', 'money', 'money');
    if (id === 'fixed-charge-coverage-ratio') return result((v.ebit + v.fixedCharges) / (v.interest + v.fixedCharges), v.ebit, v.interest + v.fixedCharges, 'EBIT', 'Fixed charges', 'number');
    return result(v.cashFlow / (v.debtService + v.interest), v.cashFlow, v.debtService + v.interest, 'Cash flow available', 'Debt service and interest', 'number');
  }
  if (id === 'loan-to-value-ltv') return result(v.loan / v.propertyValue * 100, v.loan, v.propertyValue, 'Loan amount', 'Property value', 'percent', 'money', 'money');
  if (id === 'debt-to-income-ratio') return result(v.monthlyDebt / v.monthlyIncome * 100, v.monthlyDebt, v.monthlyIncome, 'Monthly debt', 'Monthly income', 'percent', 'money', 'money');
  if (['current-ratio', 'quick-ratio', 'cash-ratio'].includes(id)) { const liquidAssets = id === 'current-ratio' ? v.currentAssets : id === 'quick-ratio' ? v.currentAssets - v.inventory : v.cash; return result(liquidAssets / v.currentLiabilities, liquidAssets, v.currentLiabilities, id === 'current-ratio' ? 'Current assets' : id === 'quick-ratio' ? 'Quick assets' : 'Cash', 'Current liabilities', 'number', 'money', 'money'); }
  if (id === 'altman-z-score') return result(1.2 * v.workingCapital / v.assets + 1.4 * v.retainedEarnings / v.assets + 3.3 * v.ebit / v.assets + 0.6 * v.marketEquity / v.liabilities + 1.0 * v.sales / v.assets, v.ebit, v.assets, 'EBIT', 'Total assets', 'number', 'money', 'money');
  if (id === 'piotroski-f-score') return result(Math.min(v.signalsPositive, v.signalsTotal), v.signalsPositive, v.signalsTotal, 'Positive signals', 'Signals tested', 'number', 'number', 'number');
  if (id === 'credit-scorecard') return result(Math.max(0, Math.min(100, 50 + (v.income / Math.max(v.debt, 1)) * 10 + v.yearsCredit * 3 - v.latePayments * 8)), v.income, v.debt, 'Annual income', 'Total debt', 'number', 'money', 'money');
  if (id === 'probability-of-default') return result(1 / (1 + Math.exp((v.creditScore - 650) / 45)) * (1 + v.debtRatio / 100 + v.latePayments * 0.05) * 100, v.creditScore, v.debtRatio, 'Credit score', 'Debt ratio', 'percent', 'number', 'percent');
  if (id === 'expected-loss') return result(v.pd / 100 * v.lgd / 100 * v.ead, v.pd, v.lgd, 'Probability of default', 'Loss given default', 'money', 'percent', 'percent');
  if (id === 'loss-given-default') return result((v.exposure - v.recovery) / v.exposure * 100, v.exposure, v.recovery, 'Exposure', 'Recovery', 'percent', 'money', 'money');
  if (id === 'exposure-at-default') return result(v.drawn + v.undrawn * v.creditConversion / 100, v.drawn, v.undrawn, 'Drawn balance', 'Undrawn commitment');
  if (['forward-contract', 'futures-p-l'].includes(id)) return result((v.spotPrice - v.contractPrice) * v.contracts * v.contractSize, v.spotPrice, v.contractPrice, 'Spot price', 'Contract price');
  const normalCdf = (value) => { const sign = value < 0 ? -1 : 1; const absolute = Math.abs(value) / Math.sqrt(2); const approximation = 1 - (((((1.061405429 * absolute + 1.453152027) * absolute + 1.421413741) * absolute + 0.39894228) * absolute + 0.31938153) * absolute) * Math.exp(-(absolute ** 2)); return 0.5 * (1 + sign * approximation); };
  if (['options-profit-loss', 'call-option-payoff', 'put-option-payoff', 'option-break-even', 'black-scholes', 'option-greeks', 'delta', 'gamma', 'theta', 'vega', 'rho'].includes(id)) {
    const volatility = v.volatility / 100;
    const riskFree = v.riskFree / 100;
    const d1 = (Math.log(v.spot / v.strike) + (riskFree + volatility ** 2 / 2) * v.years) / (volatility * Math.sqrt(v.years));
    const d2 = d1 - volatility * Math.sqrt(v.years);
    const call = v.spot * normalCdf(d1) - v.strike * Math.exp(-riskFree * v.years) * normalCdf(d2);
    const put = v.strike * Math.exp(-riskFree * v.years) * normalCdf(-d2) - v.spot * normalCdf(-d1);
    const delta = normalCdf(d1);
    const gamma = Math.exp(-(d1 ** 2) / 2) / (v.spot * volatility * Math.sqrt(v.years) * Math.sqrt(2 * Math.PI));
    const vega = v.spot * Math.exp(-(d1 ** 2) / 2) * Math.sqrt(v.years) / Math.sqrt(2 * Math.PI) / 100;
    const theta = (-v.spot * Math.exp(-(d1 ** 2) / 2) * volatility / (2 * Math.sqrt(v.years) * Math.sqrt(2 * Math.PI)) - riskFree * v.strike * Math.exp(-riskFree * v.years) * normalCdf(d2)) / 365;
    const rho = v.strike * v.years * Math.exp(-riskFree * v.years) * normalCdf(d2) / 100;
    if (id === 'call-option-payoff') return result(Math.max(0, v.spot - v.strike), v.spot, v.strike, 'Underlying price', 'Strike price');
    if (id === 'put-option-payoff') return result(Math.max(0, v.strike - v.spot), v.strike, v.spot, 'Strike price', 'Underlying price');
    if (id === 'options-profit-loss') return result(Math.max(0, v.spot - v.strike) - v.premium, call, put, 'Call value', 'Put value');
    if (id === 'option-break-even') return result(v.strike + v.premium, v.strike, v.premium, 'Strike price', 'Premium');
    if (id === 'black-scholes') return result(call, call, put, 'Call value', 'Put value');
    if (id === 'delta') return result(delta, call, put, 'Call value', 'Put value', 'number');
    if (id === 'gamma') return result(gamma, delta, vega, 'Delta', 'Vega', 'number');
    if (id === 'theta') return result(theta, call, v.years, 'Call value', 'Time to expiry', 'number');
    if (id === 'vega') return result(vega, volatility * 100, call, 'Volatility', 'Call value', 'number', 'percent', 'money');
    if (id === 'rho') return result(rho, riskFree * 100, call, 'Risk-free rate', 'Call value', 'number', 'percent', 'money');
    return result(delta + gamma + theta + vega + rho, delta, gamma, 'Delta', 'Gamma', 'number');
  }
  if (id === 'put-call-parity') return result(v.call - v.put - (v.spot - v.strike * Math.exp(-(v.riskFree / 100) * v.years)), v.call, v.put, 'Call price', 'Put price');
  ...
  if (id === 'xirr') return result(v.xirrValue, v.initialInvestment, v.totalCashFlow, 'Initial investment', 'Total cash flow', 'percent', 'money', 'money');
...
function setupXirr() {
  ...
  const scenarioMarkup = (scenario, title, initialAmount) => `<div class="xirr-scenario" data-scenario="${scenario}"><label class="xirr-scenario-name" for="xirr-name-${scenario}">${title} name</label><input id="xirr-name-${scenario}" class="xirr-name-input" value="${title}"><div class="xirr-rows" id="xirr-rows-${scenario}"></div><button class="button button-secondary xirr-add" type="button" data-scenario="${scenario}">+ Add cash flow</button><p>Scenario XIRR: <output id="xirr-${scenario}-result"></output></p></div>`;
  fieldsContainer.innerHTML = `<div class="xirr-editor"><p class="form-note">Enter any number of dated cash flows. Use negative values for investments and positive values for receipts.</p>${scenarioMarkup('a', 'Scenario A', -100000)}${scenarioMarkup('b', 'Scenario B', -100000)}</div>`;
  const addRow = (scenario, index, amount = 0, date = `20${26 + index}-01-01`, name = `Cash flow ${index}`) => { const row = document.createElement('div'); row.className = 'xirr-row'; row.dataset.scenario = scenario; row.innerHTML = `<input class="xirr-row-name" aria-label="Cash flow name" value="${name}"><input class="xirr-row-amount" aria-label="Cash flow amount" type="number" step="any" value="${amount}"><input class="xirr-row-date" aria-label="Cash flow date" type="date" value="${date}"><button class="xirr-remove" type="button" aria-label="Remove cash flow">&times;</button>`; document.querySelector(`#xirr-rows-${scenario}`).append(row); };
  ['a', 'b'].forEach((scenario) => { addRow(scenario, 0, -100000, '2026-01-01', 'Initial investment'); for (let index = 1; index < 10; index += 1) addRow(scenario, index, index === 9 ? 150000 : 0); });
  document.querySelectorAll('.xirr-add').forEach((button) => button.addEventListener('click', () => addRow(button.dataset.scenario, document.querySelectorAll(`.xirr-row[data-scenario="${button.dataset.scenario}"]`).length, 0, dateInputValue(new Date()), 'New cash flow')));
  fieldsContainer.addEventListener('click', (event) => { if (event.target.matches('.xirr-remove')) event.target.closest('.xirr-row').remove(); });
  const readScenario = (scenario) => { const rows = [...document.querySelectorAll(`.xirr-row[data-scenario="${scenario}"]`)].map((row) => ({ name: row.querySelector('.xirr-row-name').value.trim(), amountText: row.querySelector('.xirr-row-amount').value.trim(), date: new Date(`${row.querySelector('.xirr-row-date').value}T00:00:00`) })).filter((row) => row.amountText !== '' && !Number.isNaN(row.date.getTime())).map((row) => ({ ...row, amount: Number(row.amountText.replaceAll(',', '')) || 0 })); const cashFlows = rows.map((row) => row.amount); const dates = rows.map((row) => row.date); const valid = cashFlows.some((value) => value < 0) && cashFlows.some((value) => value > 0) && cashFlows.length >= 2; return { rows, cashFlows, dates, valid, rate: valid ? xirr(cashFlows, dates) : null }; };
  const renderXirr = () => { ['a', 'b'].forEach((scenario) => { const data = readScenario(scenario); const output = document.querySelector(`#xirr-${scenario}-result`); output.textContent = data.valid ? pct(data.rate) : 'Add negative and positive cash flows'; output.classList.toggle('xirr-invalid', !data.valid); document.querySelector(`#xirr-comparison-name-${scenario}`).textContent = document.querySelector(`#xirr-name-${scenario}`).value.trim() || `Scenario ${scenario.toUpperCase()}`; }); const a = readScenario('a'); const b = readScenario('b'); document.querySelector('#xirr-comparison-a').textContent = a.valid ? pct(a.rate) : 'Incomplete'; document.querySelector('#xirr-comparison-b').textContent = b.valid ? pct(b.rate) : 'Incomplete'; document.querySelector('#xirr-comparison-delta').textContent = a.valid && b.valid ? pct(a.rate - b.rate) : 'Add valid cash flows'; };
