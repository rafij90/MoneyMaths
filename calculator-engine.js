...
const input = (id, label, unit, value, step = '1') => ({ id, label, unit, value, step });
const result = (value, primary, secondary, primaryLabel, secondaryLabel, valueType = 'money') => ({ value, primary, secondary, primaryLabel, secondaryLabel, valueType });
...
function fieldsFor(id) {
  ...
  if (['recurring-deposit', 'fd', 'ppf', 'nps'].includes(id)) return [input('principal', id === 'fd' ? 'Deposit amount' : 'Starting balance', '₹', 100000, '1000'), input('monthly', 'Annual contribution', '₹', id === 'fd' ? 0 : 60000, '1000'), input('rate', 'Annual interest / return', '%', id === 'ppf' ? 7.1 : 8, '0.1'), input('years', 'Period', 'years', 10)];
  ...
  if (['retirement', 'fire', 'financial-independence'].includes(id)) return [input('monthlySpend', 'Monthly retirement spending', '₹', 60000, '1000'), input('inflation', 'Inflation rate', '%', 6, '0.1'), input('returnRate', 'Expected return', '%', 12, '0.1'), input('years', 'Years to retirement', 'years', 20)];
  ...
  if (['absolute-return', 'xirr', 'annualized-return', 'holding-period-return'].includes(id)) return [input('start', 'Starting value', '₹', 100000, '1000'), input('end', 'Ending value', '₹', 150000, '1000'), input('years', 'Holding period', 'years', 3, '0.1')];
...
function calculate(id, v) {
  ...
  if (id === 'swp') { const r = rate / 12; const months = v.years * 12; const total = r === 0 ? v.principal - v.withdrawal * months : v.principal * (1 + r) ** months - v.withdrawal * (((1 + r) ** months - 1) / r); return result(Math.max(0, total), v.withdrawal * months, Math.max(0, total), 'Total withdrawn', 'Remaining corpus'); }
  if (id === 'stp') { const months = v.years * 12; const transferred = Math.min(v.principal, v.monthly * months); return result(transferred * (1 + rate) ** v.years, transferred, transferred * ((1 + rate) ** v.years - 1), 'Total transferred', 'Estimated growth'); }
  ...
  if (id === 'recurring-deposit') { const total = annuity(v.monthly / 12, rate, v.years); return result(total, v.monthly * v.years, total - v.monthly * v.years, 'Total deposited', 'Interest earned'); }
  ...
  if (id === 'bond-yield') return result(v.coupon / v.price * 100, v.coupon, v.price, 'Annual coupon', 'Bond price', 'percent');
  ...
  if (id === 'real-return') return result(((1 + v.returnRate / 100) / (1 + v.inflation / 100) - 1) * 100, v.returnRate, v.inflation, 'Nominal return', 'Inflation', 'percent');
  if (id === 'cagr') return result(((v.end / v.start) ** (1 / v.years) - 1) * 100, v.start, v.end, 'Starting value', 'Ending value', 'percent');
  ...
  if (id === 'debt-to-income') return result(v.debt / v.income * 100, v.debt, v.income, 'Monthly debt', 'Monthly income', 'percent');
  ...
  if (['retirement', 'fire', 'financial-independence'].includes(id)) { const annualSpend = v.monthlySpend * 12 * (1 + v.inflation / 100) ** v.years; const corpus = annualSpend / (v.returnRate / 100); return result(corpus, annualSpend, v.monthlySpend * 12, 'Target corpus', 'Current annual spend'); }
  ...
  if (id === 'savings-rate') return result(v.savings / v.income * 100, v.savings, v.income, 'Monthly savings', 'Monthly income', 'percent');
  ...
  if (id === 'contribution-margin') return result((v.revenue - v.variableCosts) / v.revenue * 100, v.revenue - v.variableCosts, v.revenue, 'Contribution', 'Revenue', 'percent');
  if (id === 'gross-margin') return result((v.revenue - v.cogs) / v.revenue * 100, v.revenue - v.cogs, v.revenue, 'Gross profit', 'Revenue', 'percent');
  if (id === 'operating-margin' || id === 'net-profit-margin') return result((v.revenue - v.costs) / v.revenue * 100, v.revenue - v.costs, v.revenue, id === 'operating-margin' ? 'Operating profit' : 'Net profit', 'Revenue', 'percent');
  if (id === 'ebit') return result(v.revenue - v.costs, v.revenue, v.costs, 'Revenue', 'Operating costs');
  if (id === 'markup-vs-margin') return result((v.price - v.cost) / v.cost * 100, (v.price - v.cost) / v.price * 100, v.price - v.cost, 'Markup', 'Margin', 'percent');
  if (id === 'unit-economics') return result(v.price - v.variableCost, (v.price - v.variableCost) / v.price * 100, v.fixedCosts, 'Contribution per unit', 'Contribution margin', 'money');
  ...
  if (id === 'ebitda-margin') return result(v.ebitda / v.revenue * 100, v.ebitda, v.revenue, 'EBITDA', 'Revenue', 'percent');
  ...
  if (id === 'absolute-return') return result((v.end - v.start) / v.start * 100, v.start, v.end - v.start, 'Starting value', 'Gain / loss', 'percent');
  if (['xirr', 'annualized-return'].includes(id)) return result(((v.end / v.start) ** (1 / v.years) - 1) * 100, v.start, v.end - v.start, 'Starting value', 'Gain / loss', 'percent');
  if (id === 'holding-period-return') return result((v.end - v.start) / v.start * 100, v.start, v.years, 'Starting value', 'Holding period', 'percent');
  if (id === 'realized-vs-unrealized-return') return result((v.realized + v.current - v.invested) / v.invested * 100, v.realized, v.current, 'Realized value', 'Unrealized value', 'percent');
  if (id === 'dividend-return') return result(v.dividend / v.investment * 100, v.dividend, v.investment, 'Dividend received', 'Investment value', 'percent');
  if (id === 'total-shareholder-return-tsr') return result((v.endPrice - v.startPrice + v.dividend) / v.startPrice * 100, v.endPrice - v.startPrice, v.dividend, 'Price return', 'Dividend return', 'percent');
  if (id === 'portfolio-return') return result((v.final - v.initial + v.income) / v.initial * 100, v.initial, v.income, 'Initial value', 'Income received', 'percent');
  ...
  if (id === 'portfolio-risk') return result(v.volatility, v.portfolioReturn, v.riskFree, 'Portfolio return', 'Risk-free rate', 'percent');
  ...
  if (id === 'treynor-ratio') return result((v.portfolioReturn - v.riskFree) / v.beta, v.portfolioReturn, v.riskFree, 'Portfolio return', 'Risk-free rate', 'percent');
  if (id === 'jensens-alpha') return result(v.portfolioReturn - (v.riskFree + v.beta * (v.marketReturn - v.riskFree)), v.portfolioReturn, v.marketReturn, 'Portfolio return', 'Market return', 'percent');
  ...
  if (id === 'dividend-yield') return result(v.dividend / v.price * 100, v.dividend, v.price, 'Dividend per share', 'Share price', 'percent');
  if (id === 'dividend-payout-ratio') return result(v.dividend / v.eps * 100, v.dividend, v.eps, 'Dividend per share', 'Earnings per share', 'percent');
  if (id === 'retention-ratio') return result((v.eps - v.dividend) / v.eps * 100, v.eps, v.dividend, 'Earnings per share', 'Dividend per share', 'percent');
  if (id === 'roe') return result(v.netIncome / v.equity * 100, v.netIncome, v.equity, 'Net income', 'Shareholders equity', 'percent');
  if (id === 'roa') return result(v.netIncome / v.assets * 100, v.netIncome, v.assets, 'Net income', 'Average assets', 'percent');
  if (id === 'roic') return result(v.nopat / v.investedCapital * 100, v.nopat, v.investedCapital, 'NOPAT', 'Invested capital', 'percent');
  if (id === 'earnings-yield') return result(v.eps / v.price * 100, v.eps, v.price, 'Earnings per share', 'Share price', 'percent');
...
document.querySelector('#calculator-fields').innerHTML = fields.map((item) => `<label for="${item.id}">${item.label} <span>(${item.unit})</span></label><div class="input-wrap"><span>${item.unit === '₹' ? '₹' : item.unit === '%' ? '%' : 'N'}</span><input id="${item.id}" type="number" min="0" step="${item.step}" value="${item.value}"></div>`).join('');
function render() { const values = Object.fromEntries(fields.map((item) => [item.id, Number(document.querySelector(`#${item.id}`).value) || 0])); const output = calculate(id, values); document.querySelector('#total-value').textContent = output.valueType === 'percent' ? pct(output.value) : output.valueType === 'number' ? number(output.value) : money(output.value); document.querySelector('#invested-value').textContent = output.primaryLabel === 'Months of cover' ? `${number(output.primary)} months` : money(output.primary); document.querySelector('#returns-value').textContent = output.secondaryLabel === 'Months of cover' ? `${number(output.secondary)} months` : output.secondaryLabel === 'Monthly EMI' ? money(output.secondary) : output.valueType === 'percent' ? pct(output.secondary) : money(output.secondary); document.querySelector('#breakdown-one-label').textContent = output.primaryLabel; document.querySelector('#breakdown-two-label').textContent = output.secondaryLabel; document.querySelector('#result-caption').textContent = 'This result uses the assumptions entered above. Actual returns, rates, and costs may vary.'; }
